...
class VideoUrlParser {
  ...
  public static function get_vimeo_embed($vimeo_video_id, $autoplay = 1) {
    $embed = "https://player.vimeo.com/video/$vimeo_video_id?byline=0&amp;portrait=0";
    return $embed;
  }
  ...
  public static function get_background_video($url, $quality = '1080p', $muted = 'muted', $atts = false) {
    ...
    if ($service == 'mp4') {
      return '<div class="video' . esc_attr($video_block_class) . '"><video id="' . uniqid('video-') . '" class="jquery-background-video" playsinline ' . $muted . ' loop src="' . esc_url($url) . '" type="video/mp4" preload="none"></video></div>';
    } elseif ($service == 'youtube') {
      wp_enqueue_script('pt-youtube-video');
      
      return '<div class="video-wrap"><div class="video' . esc_attr($video_block_class) . '" data-quality="'.esc_attr($quality).'" data-muted="'.($muted ? 'true' : 'false').'" data-type="youtube" data-id="'.self::get_url_id($url).'"></div></div>';
    } elseif ($service == 'vimeo') {
      ...
      if (!empty($o_array) && isset($o_array->request->files->progressive) && count($o_array->request->files->progressive) > 0) {
        ...
        return '<div class="video' . esc_attr($video_block_class) . '"><video id="' . uniqid('video-') . '" class="jquery-background-video" playsinline ' . $muted . ' loop src="' . esc_url($result_array['url']) . '" type="' . esc_attr($result_array['mime']) . '" data-quantity="' . esc_attr($result_array['quality']) . '" preload="none"></video></div>';
      } else {
        return false;
      }
    }
